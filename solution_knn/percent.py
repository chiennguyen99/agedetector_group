...
def create_data(pre_path):
    ...
    list_file_name = read.name_file_in_folder(path_folfer=pre_path + 'get_label_for_groups')
    list_model = ["__label__18-24", "__label__25-34", "__label__35-44", "__label__45-54", "__label__55+"]
    for file_name in list_file_name:
        data_in_file = read.elements_in_each_line_of_file(path=pre_path + 'get_label_for_groups/' + file_name)
        print(file_name)
        matrix = []
        ...
        write.matrix(path_file=pre_path + 'percent/' + file_name, list_data=matrix)


def percent(pre_path):
    create_data(pre_path)
